...
| 设备 | 架构 | 文件 |
|---|---|---|
| 手机 / 平板 / 模拟器 | arm64-v8a、x86_64（同一个包） | `gkd_pure-v0.1.6.apk` |
...
示例：`https://v4.gh-proxy.org/https://github.com/cookie-kangd/gkd_pure/releases/download/v0.1.6/gkd_pure-v0.1.6.apk`
...
| `registry.npmmirror.com/@gkd-kit/docs/...` | 内置文档页取镜像地址 | 你打开内置文档页时 |
| `gkd.li` | 官方帮助文档 | 你点击「使用帮助」等入口时 |
| 用户主动发布的快照 / 日志链接 | 生成分享链接（需自备网络环境） | 你主动点「生成链接」时 |

**说明**：`gkd.li?r=N` 里的 `r` 是**官方文档站的路由参数**（`r=1`/`r=5`/`r=6` 分别对应不同帮助页），
不是引荐追踪码 —— 去掉会导致帮助链接全部跳到同一个页面，因此保留。
...
**v0.1.6 具体内容**见 [CHANGELOG.md](./CHANGELOG.md)。本版本为 v0.1 系列最终版。
...
4. 推送 `main`，再推 tag：`git tag v0.1.6 && git push origin v0.1.6`
